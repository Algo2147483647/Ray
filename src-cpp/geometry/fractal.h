...
void Tree3D(vector<double>& center, vector<double>& direction,
	...
	vector<double> new_x, new_y, p, new_direction, new_center;
	...
	for (int i = 0; i < forks; i++) {
		...
		new_direction = rotateMat * p;
		new_center = length * new_direction + center;

		edSet.push_back(new_center);
		stSet.push_back(center);

		Tree3D(new_center, new_direction, length * scale, angle, scale, forks, level - 1, stSet, edSet);
	}
}
