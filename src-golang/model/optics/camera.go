...
func (c *Camera) GenerateRay(ray *Ray, row, col int) *Ray {
	...
	case 4:
		...
		third := math_lib.Normalize(math_lib.Cross4(dir, up, tempVec)) // 计算第3个基向量，使其与dir和up正交
		right := math_lib.Normalize(math_lib.Cross4(dir, up, third))   // 计算右向量（与dir和up正交）

		// 正交化所有向量，确保它们互相垂直
		res := math_lib.GramSchmidt(
			mat.VecDenseCopyOf(dir),
			mat.VecDenseCopyOf(up),
			mat.VecDenseCopyOf(third),
			mat.VecDenseCopyOf(right))
		_, upOrtho, _, rightOrtho := res[0], res[1], res[2], res[3]
		right = rightOrtho // 使用正交后的右向量和上向量构建视图方向

		ray.Color = mat.NewVecDense(3, []float64{1, 1, 1})
		ray.Origin.CloneFromVec(c.Position)
		ray.Direction.CloneFromVec(dir)
		ray.Direction.AddScaledVec(ray.Direction, u*halfWidth, right) // 使用正交后的右向量和上向量构建视图方向
