...
func ColorFunc1(ray *Ray, norm *mat.VecDense) *mat.VecDense {
	if math.Abs(math.Abs(norm.AtVec(0))-1) < math_lib.EPS {
		return mat.NewVecDense(3, []float64{1, 0.5, 0.5})
	} else if math.Abs(math.Abs(norm.AtVec(1))-1) < math_lib.EPS {
		return mat.NewVecDense(3, []float64{0.5, 1, 0.5})
	} else if math.Abs(math.Abs(norm.AtVec(2))-1) < math_lib.EPS {
		return mat.NewVecDense(3, []float64{0.5, 0.5, 1})
	} else if math.Abs(math.Abs(norm.AtVec(3))-1) < math_lib.EPS {
