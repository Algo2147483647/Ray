...
func (p *QuadraticEquation) Intersect(raySt, rayDir *mat.VecDense) float64 {
	...
	c := mat.Dot(raySt, linear_algebra.MulVec(t, p.A, raySt)) + mat.Dot(p.B, raySt) + p.C // 计算常数项: raySt^T A raySt + b^T raySt + c
	t1, t2, count := basic_algebra.SolveQuadraticEquationReal(a, b, c)                    // 解二次方程 a*t^2 + b*t + c = 0
