...
// TracePixel 追踪单个像素
func (h *Handler) TracePixel(camera camera.Camera, objTree *object.ObjectTree, row, col, samples int64) *mat.VecDense {
	color := mat.NewVecDense(3, nil)
	ray := h.RayPool.Get().(*optics.Ray)
	defer h.RayPool.Put(ray)

	for s := int64(0); s < samples; s++ {
		ray = camera.GenerateRay(ray, row, col)    // build ray
