...
func parseImplicitEquation(objDef map[string]interface{}) ([]shape.Shape, error) {
	...
	function, gradient, err := buildImplicitFunction(functionName, objDef, center, scale)
	if err != nil {
		return nil, err
	}
...
func buildImplicitFunction(
	...
	case "gyroid":
		frequency, offset, err := parseImplicitGyroidParameters(objDef)
		if err != nil {
			return nil, nil, err
		}
	...
	default:
		return nil, nil, fmt.Errorf("unsupported implicit function %q", name)
	}
}
