...
func CosineSampleHemisphere(sample Sample2D) Direction {
	r := math.Sqrt(ClampUnit(sample.U))
	theta := 2 * math.Pi * ClampUnit(sample.V)
	x := r * math.Cos(theta)
	y := r * math.Sin(theta)
...
func CosineSampleHemisphereND(sample Sample2D, dim int) Direction {
	...
	radius := math.Pow(ClampUnit(uniforms[0]), 1/float64(tangentDim))
...
func deterministicUniforms(sample Sample2D, count int) []float64 {
	seed := uint64(ClampUnit(sample.U)*float64(^uint64(0)>>1)) ^
		(bitsRotateLeft64(uint64(ClampUnit(sample.V)*float64(^uint64(0)>>1)), 32))
	if seed == 0 {
...
func uniformSphereDirection(dim int, uniforms []float64) []float64 {
	...
	sum := 0.0
	for i := 0; i < dim; i += 2 {
		u1 := math.Max(1e-12, ClampUnit(uniforms[i%len(uniforms)]))
		u2 := ClampUnit(uniforms[(i+1)%len(uniforms)])
